...
const createCube = (xMin, xMax, yMin, yMax, zMin, zMax, labelFace, colorIndex) => {
  ...
  const surfaces = [
    // All sides (for the lines)
    {
      type: 'scatter3d',
      mode: 'lines',
      x: [xMin, xMax, xMax, xMin, xMin, xMin, xMin, xMin, xMin, xMax, xMax, xMin, xMin, xMax, xMax, xMax, xMax],
      y: [yMin, yMin, yMax, yMax, yMin, yMin, yMax, yMax, yMax, yMax, yMin, yMin, yMax, yMax, yMax, yMin, yMin],
      z: [zMin, zMin, zMin, zMin, zMin, zMax, zMax, zMin, zMax, zMax, zMax, zMax, zMax, zMax, zMin, zMin, zMax],
      line: {
        color: 'red',
        width: 4,
      },
  ...
  if (labelFace) {
    ...
    surfaces.push({
      ...
      showscale: false,
      opacity: 1.0,
      colorscale: [[0, faceColor], [1, faceColor]],  // Different color for the label face
    });
...
const ResultsDisplay = ({ solution, palletDims }) => {
  ...
  console.log("Component rendering", solution.length);
  ...
  useEffect(() => {
    const handleKeyPress = (event) => {
      if (event.key === 'ArrowRight') {
        ...
        setCurrentIndex((prevIndex) => (prevIndex - 1 + solution.length) % solution.length);
      }
    };
  ...
  const data = currentSolution.map((box) => {
    ...
    const labelFace = getLabelFace(x, x + l, y, y + w, z, z + h, box.orientation);
  ...
  return (
    <div>
      <h2>Results (Pallet {currentIndex + 1} of {solution.length})</h2>
      <Plot
        data={data}
        layout={{
          width: 1200,
          height: 750,
          title: `Palletization Visualization (Pallet ${currentIndex + 1})`,
          scene: {
